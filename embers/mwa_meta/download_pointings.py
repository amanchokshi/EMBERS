...
def wget_meta(start, stop, num_pages, out_dir):
    ...
    for npg in range(num_pages):
        time.sleep(10)
        cerberus_url = f"http://ws.mwatelescope.org/metadata/find?mintime={start_gps}&maxtime={stop_gps}&extended=1&page={npg+1}"
        print(cerberus_url)
        print(f"Downloading page {npg+1} of metadata")
