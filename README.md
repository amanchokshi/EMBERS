...
FEKO simulations were run to generate FEE beam models for the reference antennas. A model was generated for the XX & YY polarization of the reference antennas. 

```
cd ./data/FEE_Reference_Models/
```

The `.ffe` files generated by FEKO need to be converted to something that we can use. `project_beam2healpix.py` finds the power in the beam, interpolates and rotates it into a healpix projection.

```
python project_beam2healpix.py
```
This will create `ref_dipole_models.npz` and a diagnostic plot of the beams. 



### Divide Out Satellites

Working on sat chan detection ......
