...
class Test_batch_align(unittest.TestCase):
    ...
    def test_save_aligned(self):   
        ...
        aligned_npz = Path(f'{self.date}/{self.timestamp}/{self.ref}_{self.aut}_{self.timestamp}_aligned.npz')
        aligned_data = np.load(aligned_npz)
        self.assertTrue(aligned_npz.is_file())
        self.assertTrue(aligned_data['ref_p_aligned'].shape, aligned_data['tile_p_aligned'].shape)
